...
### 5.4 Canvas Micro-Interactions

- 沉浸式画布 `chat input` 固定为三段结构：顶部素材轨道、文本输入区、底部参数条。
- 当顶部素材轨道出现待确认图片、已确认图片或添加图片入口时，素材轨道与文本输入区之间只允许保留极窄呼吸间距，不允许出现明显空白带。
- 当画布输入框处于空态且顶部只有“添加图片”入口时，不得再额外预留一整行附件轨道高度；文本输入区应直接贴近加号卡片下缘开始排版。
- 文本输入区与底部参数条同样保持紧凑贴合，参数条只保留必要分隔线和最小安全间距，不得把输入框整体继续向下撑开。
- 空态与附件态下，文本输入区内容都必须从输入区上沿开始布局，不允许因为底部发送按钮对齐而把 placeholder 或已输入文本整体压到下半区。
- [已完成 2026-04-18] 沉浸式画布 `chat input` 的文字区默认只保留两行文字高度；短文本始终钳制在双行基线内，只有当文本真实超过两行时才允许向上扩展，不得默认占出接近三行的空白。
- [已完成 2026-04-18] `data-testid="chat-input-immersive-input-row"` 对应的输入行容器不得继续 `flex` 吞掉剩余可用高度；该层必须按 textarea 与发送按钮的真实内容高度收紧，只保留必要安全间距。
- [已完成 2026-04-18] 沉浸式空态 `chat input` 外壳不得再固定保留大块空白高度；`272px` 只可作为上限而非默认高度，空态 shell 必须按“素材轨道 + 文本区 + 参数条”的真实内容高度收紧。
- 画布底部 `模版` 按钮展开的模板浏览器必须锚定在按钮附近显示，默认优先贴按钮上方展开；只有上方空间不足时才允许翻到下方，且不得出现固定大偏移导致弹层脱离按钮的情况。
- 画布内图片被左键选中后，图片上方悬浮工具条的底边必须贴近原生旋转把手安全区，只让出旋转把手所需距离，不允许远离图片漂浮。
- 图片位置变化时，悬浮工具条必须同步跟随；取消选中、按 `Esc`、或切换到其他操作后，悬浮工具条立即消失。
